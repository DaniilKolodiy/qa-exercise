...
namespace TestProject.Helpers
{
    ...
    public sealed class ExpectedConditions
    {
        ...
        /// <summary>
        /// An expectation for the URL of the current page to be end with specific path.
        /// </summary>
        /// <param name="url">The URL path that the specific page should be on</param>
        /// <returns><see langword="true"/> when the URL is what it should be; otherwise, <see langword="false"/>.</returns>
        public static Func<IWebDriver, bool> UrlEndsWith(string url)
        {
            return (driver) => { return driver.Url.ToLowerInvariant().EndsWith(url.ToLowerInvariant()); };
        }
